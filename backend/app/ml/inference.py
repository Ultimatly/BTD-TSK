...
def _compose_diagnosis_texts(
    ...
    if stage_map:
        if secondary_text:
            summary = f'????? {dominant_text} ?????? {secondary_text}??????? {focus_text} ?????'
        else:
            summary = f'????? {dominant_text} ????????? {focus_text} ?????'
    else:
        summary = f'????? {_format_stage_cn(dominant, with_stage=True)} ????????? {_format_stage_cn(focus, with_stage=True)} ?????'
    ...
    if risk == '???':
        ...
        advice = (
            f'???????????????????????? {_format_stage_cn(focus, with_stage=True)} ?????????'
        )
